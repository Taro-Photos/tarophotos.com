...
// FORK (TARO): allow H1-H6 through the SCREAMING_CASE filter — the journal
// prose kit exports <H2> (a real component), which the stock regex misreads
// as an all-caps constant. Everything else is byte-identical to the bundled lib.
export const isComponentName = (n) => !n.endsWith('Props')
  && (/^H[1-6]$/.test(n) || !/^[A-Z][A-Z0-9_]+$/.test(n))
