...
- `Form`：SES の設定（`SES_REGION` / `SES_FROM_EMAIL` / `SES_AWS_ROLE_ARN`）と通知先メール（`SES_TO_EMAIL`）は本番では GitHub repository variables → Cloud Run の runtime env、ローカルでは `.env.local` で管理。送信者にも自動返信メールを返して受付完了を伝える。
- `CalendarConnect`（任意）
